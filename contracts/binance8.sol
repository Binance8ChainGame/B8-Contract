...
contract Binance8 is ERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0xcDeD26161d17B62Cc1A8ec9281Ed06c15FDEc3B1;
    ...
    constructor() public ERC20("binance8", "B8") {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BABYBUSD: The dividend tracker already has that address");

        BUSDDividendTracker newDividendTracker = BUSDDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BABYBUSD: The new dividend tracker must be owned by the BABYUSD token contract");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BABYBUSD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BABYBUSD: Cannot update gasForProcessing to same value");
...
contract BUSDDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("binance8_Dividen_Tracker", "binance8_Dividen_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "binance8_Dividen_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "binance8_Dividen_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BABYCAKE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "binance8_Dividen_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "binance8_Dividen_Tracker: Cannot update claimWait to same value");
